...
import { TrashIcon, PlusIcon, PencilAltIcon } from "@heroicons/react/outline";
import Head from "next/head";
import AddUserDialog from "../../components/layouts/createSchools";
import Enroll from "../../components/layouts/Enroll";
import EditUserDialog from "../../components/layouts/editSchools";
import ConfirmDialog from "../../components/ui/confirm";
const axios = require("axios");
import { API_URL } from "../../config/constants";
import Template from "../../components/layouts/admin";
...
export default function Assets() {
  ...
  useEffect(() => {
    axios
      .post(API_URL + "schools/get_schools.php", {
  ...
  function deleteUserFunc(id) {
    setDeleteUser(false);
    axios
      .post(API_URL + "schools/delete_schools.php", {
        id: id,
      })
      .then(function (response) {
        if (response.data?.meta?.error) {
  ...
  return (
    <Template page="Schools">
                      ...
                      <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                        <table className="min-w-full divide-y divide-gray-200">
                          <thead className="bg-gray-50">
                            <tr>
                              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                <span class="md:table-cell hidden">School</span> <span class="md:hidden"> Data</span>
                              </th>
                                    ...
                                    <div className="flex items-center">
                                      <div className=" space-y-2 md:space-y-0">
                                        <div className="flex w-2">
                                          <span class="md:hidden text-sm text-gray-900 font-medium">SchoolName :</span>
                                          <div className="text-sm text-gray-900 ml-1">{person?.SchoolName}</div>
                                        </div>

                                        <div className="flex ">
                                          <span className="md:hidden text-sm text-gray-900 font-medium">State :</span>
                                          <div className="md:hidden text-sm text-gray-900 ml-1">{person?.State}</div>
                                        </div>
                                        {/* <div className="flex ">
                                          <span className="md:hidden text-sm text-gray-900 font-medium">Coordinator :</span>
                                          <div className="md:hidden text-sm text-gray-900 ml-1">{person?.Coordinator}</div>
                                        </div> */}
                                        {/* <div className="flex ">
      ...
      Check 
                                  ...
                                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 hidden md:table-cell">{person?.State}</td>
                                  {/* <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 hidden md:table-cell">{person?.Coordinator}</td> */}
